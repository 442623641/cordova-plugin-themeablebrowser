...
- (void)show:(CDVInvokedUrlCommand*)command withAnimation:(BOOL)animated
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (self.themeableBrowserViewController != nil) {
            ...
            //stru,
            [self.viewController.navigationController pushViewController:self.themeableBrowserViewController animated:YES];
        }
    });
}
...
- (void)emitEventForButton:(NSDictionary*)buttonProps withIndex:(NSNumber*)index
{
    if (buttonProps) {
        ...
        if (event) {
            ...
            if([self.navigationDelegate.themeableBrowserViewController.currentURL absoluteString]){
                //strum
